  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (result && result.url) {
        setFoodForm(prev => ({ ...prev, image_url: result.url }));
        setUploadError(null);
      } else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSaveFood} className="space-y-3">
              ...
              <div>
                <label className="text-xs font-bold text-slate-300 block mb-1">Dish Photo (Supabase Storage)</label>
                    ...
                    {isUploadingImage ? (
                      ...
                      <div className="w-full flex items-center justify-between gap-3">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setFoodForm(prev => ({ ...prev, image_url: '' }));
